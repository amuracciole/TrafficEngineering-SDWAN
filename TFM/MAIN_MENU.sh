...
_Config() {
    ...
    then
        ...
        then
            ...
            then
                ...
                if [ $opt_conf_path -eq "1" ]
                then
                    sh All_IPv6_long_path.sh
                    sleep 1
                
                #CAMINO IPv6 CORTO
                elif [ $opt_conf_path -eq "2" ]
                then
                    sh All_IPv6_short_path.sh
                    sleep 1
                fi
            
            #CONFIGURAR ALGUN TRAFICO IPv6
            elif [ $opt_conf_IP -eq "2" ]
            then
                clear
                _Banner
                ...
                if [ $opt_conf_path -eq "1" ]
                then
                    sh Some_IPv6_long_path.sh $src_IP $dst_IP
                    sleep 1
                
                #CAMINO IPv6 CORTO
                elif [ $opt_conf_path -eq "2" ]
                then
                    sh Some_IPv6_short_path.sh $src_IP $dst_IP
                    sleep 1
                fi
            fi
        fi
    ...
    then
        ...
        _InitMessage
        _SelectIPVersion
        read opt_select_version
        if [ $opt_select_version -eq "1" ]
        then
            _SelectConmDel
        ...
        then
            ...
            then
                ...
                _SelectActionA
                read action
                sh Delete_some_IPv6_flow_A.sh $selec_src_IP $select_dst_IP $in_port $action
            elif [ $opt_select_conm_del -eq "2" ]
            then
                sh Dump_flows.sh CONM_B
                #_SelectPriority
                ...
                _SelectActionB
                read action
                sh Delete_some_IPv6_flow_B.sh $selec_src_IP $select_dst_IP $in_port $action
            elif [ $opt_select_conm_del -eq "3" ]
            then
                sh Dump_flows.sh CONM_C
                #_SelectPriority
                ...
                _SelectActionC
                read action
                sh Delete_some_IPv6_flow_C.sh $selec_src_IP $select_dst_IP $in_port $action
            elif [ $opt_select_conm_del -eq "4" ]
            then
                sh Dump_flows.sh CONM_D
                #_SelectPriority
                ...
                _SelectActionD
                read action
                sh Delte_some_IPv6_flow_D.sh $selec_src_IP $select_dst_IP $in_port $action
            elif [ $opt_select_conm_del -eq "5" ]
            then
                sh Dump_flows.sh CONM_E
                #_SelectPriority
                ...
                _SelectActionE
                read action
                sh Delete_some_IPv6_flow_E.sh $selec_src_IP $select_dst_IP $in_port $action
            fi
        fi
    fi
}
