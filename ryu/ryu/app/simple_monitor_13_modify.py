...
class SimpleMonitor13(simple_switch_13.SimpleSwitch13):
	...
	def show_stat(self, _type):
		...
		if _type == 'flow':
			print('    datapath     ''   in-port       eth-src            eth-dst       ''  packets''   eth_type')
			print('---------------- ''  -------- ''-----------------   ----------------- ''  --------- '' ---------')
			for dpid in bodys.keys():
				for stat in sorted([flow for flow in bodys[dpid] if flow.priority == 1],
						   key=lambda flow: (flow.match.get('in_port'), flow.match.get('eth_src'), flow.match.get('eth_dst'), flow.match.get('eth_type'))):
					if (stat.match.get('in_port') != None and stat.match.get('eth_src') != None and stat.match.get('eth_dst') != None):
