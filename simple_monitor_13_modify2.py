...
class SimpleMonitor13(simple_switch_13.SimpleSwitch13):
	...
	def show_stat(self, _type):
		...
		if _type == 'flow':
			print('    datapath     ''   in-port       ipv4-src            ipv4-dst       ''  packets''   eth_type')
			print('---------------- ''  -------- ''-----------------   ----------------- ''  --------- '' ---------')
			for dpid in bodys.keys():
				for stat in sorted([flow for flow in bodys[dpid] if flow.priority == 1],
						   key=lambda flow: (flow.match.get('in_port'), flow.match.get('ipv4_src'), flow.match.get('ipv4_dst'), flow.match.get('eth_type'))):
					if (stat.match.get('in_port') != None and stat.match.get('ipv4_src') != None and stat.match.get('ipv4_dst') != None):
						if (stat.match.get('eth_type') == 2048): 
							eth_type_word = "IPv4"
						if (stat.match.get('eth_type') == 2054): 
							eth_type_word = "ARP"
						if (stat.match.get('eth_type') == 34525): 
							eth_type_word = "IPv6"
						if (stat.match.get('eth_type') == 35020): 
							eth_type_word = "LLDP"
						else :
							eth_type_word = "Other"
						print('%016x %8s %19s %19s %10s %11s' % (
							dpid,
