...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def show_stat(self, type):
        ...
        if(type == 'port'):
            print('datapath             port  '
                  '  port-speed(Kbps)   port-freebw (Kbps)'
                  '    max-capacity(Kbps)  '
                  '  port-stat     link-stat')
            print('----------------   -------- '
                  '-----------------  -------------------   --------------------   '
            ...
            for dpid in bodys.keys():
                for stat in sorted(bodys[dpid], key=attrgetter('port_no')):
                    if stat.port_no != ofproto_v1_3.OFPP_LOCAL:
                        #print (dpid != 2560)
                        if ((dpid != 2560 or stat.port_no != 3) and (dpid != 3584 or stat.port_no != 3)):
                            print(format % (
                                dpid, stat.port_no,
                            ...
                            file.write("0" + ",%x" %dpid + ",%x" %stat.port_no + ",%f" %(setting.MAX_CAPACITY - self.free_bandwidth[dpid][stat.port_no]))
                            file.write("\n")
            print '\n'
